...
void lp_Print(void (*output)(void *, char *, int), void * arg, char *fmt, va_list ap)
{
    ...
    */
    ...
    for(;;) {
		...
		{ 
		    /* scan for the next '%' */
			for (fmt_pointer = fmt; *fmt_pointer && *fmt_pointer != '%'; fmt_pointer++);
		    /* flush the string found so far */
			OUTPUT(arg, fmt, fmt_pointer - fmt);
			fmt = fmt_pointer;
		...
		if (*fmt == '0') padc = '0', fmt++;
		for (width = 0; *fmt && IsDigit(*fmt); fmt++) width = 10*width + Ctod(*fmt);
		/* check for other prefixes */
		if (*fmt == '.') {
			fmt++;
			while(IsDigit(*fmt)) prec = 10*prec + Ctod(*fmt++);
		} else {
			prec = 6;
		}
		/* check format flag */
		...
		switch (*fmt) {
			case 'b':
			    if (longFlag) num = va_arg(ap, long int);
